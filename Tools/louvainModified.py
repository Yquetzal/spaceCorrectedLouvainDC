...
def computeSumWeightsByCom(partition,graph):
...
def modularity(partition, graph,nullModel) :
	...
	sumWeightsByComOriginal = computeSumWeightsByCom(partition,graph)
	sumWeightsByComNullMod = computeSumWeightsByCom(partition, nullModel)
...
def getPartitionAtSpecificLevel(graph, nullModel, partition = None, levelToREturn=0) :
	dendo = generate_dendrogram(graph, nullModel, partition)
	return partition_at_level(dendo, levelToREturn)
...
def generate_dendrogram(graph, nullModel, part_init = None) :
	...
	if round(graph.size(weight="weight")) != round(nullModel.size(weight="weight")):
		raise Exception("ERROR, the null model and the original network have different size: ORIGINAL: %s NULLmodel: %s"%(graph.size(weight="weight"),nullModel.size(weight="weight")))
	...
	current_Null = induced_graph(partition,current_Null)
...
class Status :
	...
	def init(self, graph, nullModel, part = None) :
		...
		if part == None :#if it is the first iteration, put each node in its own community, intern edges are only loops
			...
			self.internals = computeSumWeightsByCom(part,graph)
			self.internalsNull = computeSumWeightsByCom(part,nullModel)
